...
def manual_forward_mlp(model_0, model_1, x, alpha=0.2):
    ...
    # Layer 1
    weight_diff1 = model_1.fc1.weight - model_0.fc1.weight
    bias_diff1 = model_1.fc1.bias - model_0.fc1.bias
    # x = torch.relu(torch.matmul(x, weight_diff1.t()) + bias_diff1)

    # Layer 2
    weight_diff2 = model_1.fc2.weight - model_0.fc2.weight
    bias_diff2 = model_1.fc2.bias - model_0.fc2.bias
    # x = torch.relu(torch.matmul(x, weight_diff2.t()) + bias_diff2)

    # Layer 3
    weight_diff3 = model_1.fc3.weight - model_0.fc3.weight
    bias_diff3 = model_1.fc3.bias - model_0.fc3.bias
    # x = torch.matmul(x, weight_diff3.t()) + bias_diff3
    
    #calculate sparse loss
    weight_diff1_flatten = weight_diff1.view(-1)
    bias_diff1_flatten = bias_diff1.view(-1)
    weight_diff2_flatten = weight_diff2.view(-1)
    bias_diff2_flatten = bias_diff2.view(-1)
    weight_diff3_flatten = weight_diff3.view(-1)
    bias_diff3_flatten = bias_diff3.view(-1)
    
    #sparse loss using Lasso
    sparse_loss = torch.sum(torch.abs(weight_diff1_flatten)) + torch.sum(torch.abs(bias_diff1_flatten)) + torch.sum(torch.abs(weight_diff2_flatten)) + torch.sum(torch.abs(bias_diff2_flatten)) + torch.sum(torch.abs(weight_diff3_flatten)) + torch.sum(torch.abs(bias_diff3_flatten))
...
def manual_forward_single_model_mlp(model_0, x):
    ...
    #calculate sparse loss
    weight1_flatten = weight1.view(-1)
    bias1_flatten = bias1.view(-1)
    weight2_flatten = weight2.view(-1)
    bias2_flatten = bias2.view(-1)
    weight3_flatten = weight3.view(-1)
    bias3_flatten = bias3.view(-1)
    
    #sparse loss using Lasso
    sparse_loss = torch.sum(torch.abs(weight1_flatten)) + torch.sum(torch.abs(bias1_flatten)) + torch.sum(torch.abs(weight2_flatten)) + torch.sum(torch.abs(bias2_flatten)) + torch.sum(torch.abs(weight3_flatten)) + torch.sum(torch.abs(bias3_flatten))
        
    return x, sparse_loss

def manual_forward_cnn(model_0, model_1, x, alpha=0.2):
    # Convolutional Layer 1
    weight_diff1 = model_1.conv1.weight - model_0.conv1.weight
    bias_diff1 = model_1.conv1.bias - model_0.conv1.bias
    ...
    weight_diff2 = model_1.conv2.weight - model_0.conv2.weight
    bias_diff2 = model_1.conv2.bias - model_0.conv2.bias
    ...
    weight_diff3 = model_1.fc1.weight - model_0.fc1.weight
    bias_diff3 = model_1.fc1.bias - model_0.fc1.bias
    x = F.linear(x, weight_diff3, bias_diff3)
    x = F.relu(x)

    # Fully Connected Layer 2
    weight_diff4 = model_1.fc2.weight - model_0.fc2.weight
    bias_diff4 = model_1.fc2.bias - model_0.fc2.bias
    ...
    weight_diff3_flatten = weight_diff3.view(-1)
    bias_diff3_flatten = bias_diff3.view(-1)
    weight_diff4_flatten = weight_diff4.view(-1)
    bias_diff4_flatten = bias_diff4.view(-1)
    
    #sparse loss using Lasso
    sparse_loss = torch.sum(torch.abs(weight_diff1_flatten)) + torch.sum(torch.abs(bias_diff1_flatten)) + torch.sum(torch.abs(weight_diff2_flatten)) + torch.sum(torch.abs(bias_diff2_flatten)) + torch.sum(torch.abs(weight_diff3_flatten)) + torch.sum(torch.abs(bias_diff3_flatten)) + torch.sum(torch.abs(weight_diff4_flatten)) + torch.sum(torch.abs(bias_diff4_flatten))
...
def manual_forward_single_model_cnn(model_0, x):
    ...
    weight3_flatten = weight3.view(-1)
    bias3_flatten = bias3.view(-1)
    weight4_flatten = weight_diff4.view(-1)
    bias4_flatten = bias_diff4.view(-1)
    
    #sparse loss using Lasso
    sparse_loss = torch.sum(torch.abs(weight1_flatten)) + torch.sum(torch.abs(bias1_flatten)) + torch.sum(torch.abs(weight2_flatten)) + torch.sum(torch.abs(bias2_flatten)) + torch.sum(torch.abs(weight3_flatten)) + torch.sum(torch.abs(bias3_flatten)) + torch.sum(torch.abs(weight4_flatten)) + torch.sum(torch.abs(bias4_flatten))
...
def parse_args():
    ...
    parser.add_argument('--control_weight', type=float, default=0.2)
...
if __name__ == "__main__":
    ...
    for epoch in range(args.num_epoch):
        ...
        for batch_idx, ((data_target, target_target), (data_control, target_control)) in enumerate(zip(cycle(sub_train_loader_target_indices), sub_train_loader_control_indices)):
            ...
            if(args.arch == 'mlp'):
                target_output, _ = manual_forward_single_model_mlp(model_theta_0, data_target)
                # control_output = manual_forward_mlp(model_theta_1, model_theta_0, data_control, args.alpha)
                _, target_sparse_loss = manual_forward_mlp(model_theta_1, model_theta_0, data_target, args.alpha)
            elif(args.arch == 'cnn'):
                target_output, _ = manual_forward_single_model_cnn(model_theta_0, data_target)
                # control_output = manual_forward_cnn(model_theta_1, model_theta_0, data_control, args.alpha)
                _, target_sparse_loss = manual_forward_cnn(model_theta_1, model_theta_0, data_target, args.alpha)
            ...
            loss = loss_target + args.control_weight*target_sparse_loss
            loss.backward()
            optimizer.step()
            
            total_target_loss += loss_target.item()
            # total_control_loss += loss_control.item()
            total_sparse_loss += target_sparse_loss.item()
            total_loss += loss.item()
        
        print(f"Epoch {epoch}, Target Loss: {total_target_loss/(batch_idx+1)}, Sparse Loss: {total_sparse_loss/(batch_idx+1)}, Total Loss: {total_loss/(batch_idx+1)}")
